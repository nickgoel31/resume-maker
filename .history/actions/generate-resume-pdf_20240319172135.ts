...
export const generateResumePDF = (values: z.infer<typeof formSchema>, experiences: Experience[], educations: Education[], profileUrl: string) => {
  ...
  if (experiences.length > 0) {
    ...
    experiences.forEach(experience => {
      const month = experience.startDate..toLocaleString('default', { month: 'long' });
      content.push(
        { text: `Job Title: ${experience.jobTitle}` },
        { text: `Company: ${experience.company}` },
        { text: `Description: ${experience.description}` },
        { text: `Start Date: ${experience.startDate?.getFullYear()}, ${experience.startDate?.getMonth()}` },
        { text: `End Date: ${experience.endDate?.getFullYear()}, ${experience.endDate?.getUTCMonth()}` }
      );
    });
  }
