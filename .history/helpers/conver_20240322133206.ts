export function ConvertToSvg(profileUrl: string): string {
    ...
    const svgMarkup = `
        <svg width="${circleSize}" height="${circleSize}" viewBox="0 0 ${circleSize} ${circleSize}" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect width="${circleSize}" height="${circleSize}" fill="#cccccc50" clip-path="url(#circleClip)" />
            <image href="${profileUrl}" x="0" y="0" width="${circleSize}" height="${circleSize}" clip-path="url(#circleClip)" />
        </svg>
        <style>
            image {
                object-fit: cover;
