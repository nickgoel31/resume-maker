...
export const ThemeOne = async (values: z.infer<typeof formSchema>, page: PDFPage) => {
  ...
  const summaryLines = wordWrap(values.summary, 100, page); // Adjust width as needed
  let summaryY = page.getHeight() - 220;
...
function wordWrap(text: string, maxWidth: number, page:PDFPage): string[] {
  ...
  for (let i = 1; i < words.length; i++) {
    const word = words[i];
     const width = 1000; // Assuming font size of 12
    if (width < maxWidth) {
      currentLine += " " + word;
    } else {
...
function drawTextWithWrap(page: PDFPage, text: string, x: number, y: number, size: number, color: any) {
  const lines = wordWrap(text, 500,page); // Adjust width as needed
